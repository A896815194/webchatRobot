...
class DouyinLiveWebFetcher:
    ...
    def start(self):
        if self.start_time is None:
            ...
            if not os.path.exists("logs"):
                os.mkdir("logs")
            if not os.path.exists("logs/" + self.live_id):
                os.mkdir("logs/" + self.live_id)

            start_time_str = time.strftime('%Y%m%d_%H%M%S', self.start_time)
            self.filename = f"logs/{self.live_id}/{start_time_str}.txt"
            # 写入文件头部数据
    ...
    def _wsOnOpen(self, ws):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
                file.write(f"WebSocket connected.\n")
    ...
    def _wsOnError(self, ws, error):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"WebSocket error\n")

    def _wsOnClose(self, ws, *args):
        print("WebSocket connection closed.")
        with open(self.filename, 'a', encoding='UTF-8') as file:
                file.write(f"WebSocket connection closed.\n")
        try:
                response = requests.post(self.notify_url, json={"type":2})
                response.raise_for_status()
                print(f"【直播间链接终端触发命令重试成功】")
                with open(self.filename, 'a', encoding='UTF-8') as file:
                    file.write(f"【直播间链接终端触发命令重试成功】\n")
        except requests.exceptions.RequestException as e:
                print(f"Error during API call: {e}")
                print(f"【【直播间链接终端触发命令发送失败】")
                with open(self.filename, 'a', encoding='UTF-8') as file:
                    file.write(f"【直播间链接终端触发命令发送失败】\n")
    ...
    def _parseGiftMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【礼物msg】{user_name} 送出了 {gift_name}x{gift_cnt}\n")
    ...
    def _parseLikeMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【点赞msg】{user_name} 点了{count}个赞\n")
    ...
    def _parseMemberMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【进场msg】[{user_id}][{gender}]{user_name} 进入了直播间\n")
    ...
    def _parseSocialMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【关注msg】[{user_id}]{user_name} 关注了主播\n")
    ...
    def _parseRoomUserSeqMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【统计msg】当前观看人数: {current}, 累计观看人数: {total}\n")
    ...
    def _parseRoomStatsMsg(self, payload):
        ...
        with open(self.filename, 'a', encoding='UTF-8') as file:
            file.write(f"【直播间统计msg】{display_long}\n")
